...
from tensorflow.keras.applications import VGG19 # NOVO IMPORT para VGG19
import numpy as np
import os
import random
import pandas as pd
...
# 1. Definição do Modelo VGG19 (Transfer Learning) 
def create_vgg19_model(input_shape=(IMG_HEIGHT, IMG_WIDTH, 3), num_classes=NUM_CLASSES):
    
    # 1. Carregar a Base VGG19 (Congelar os pesos)
    base_model = VGG19(
    ...
    model = Sequential([
        base_model, # Inclui a base VGG19
...
for run in range(len(SEEDS)):
    ...
    cnn_model_no_aug = create_vgg19_model() # USANDO VGG19
    ...
    result = evaluate_model(cnn_model_no_aug, test_generator, "VGG19 SEM Data Aug", seed)
    results_no_aug.append(result)

    #CENÁRIO 2: COM DATA AUGMENTATION 
    set_seeds(seed) 
    cnn_model_with_aug = create_vgg19_model() # USANDO VGG19
    ...
    result = evaluate_model(cnn_model_with_aug, test_generator, "VGG19 COM Data Aug", seed)
    results_with_aug.append(result)

# 4. Cálculo e Apresentação Final (VGG19)
